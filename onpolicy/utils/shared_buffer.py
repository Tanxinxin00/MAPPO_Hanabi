...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.old_logits = np.zeros((self.episode_length, self.n_rollout_threads, num_agents, act_space.n), dtype=np.float32)    
        #print("old logits shape", self.old_logits.shape)
    ...
    def chooseinsert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
                     value_preds, rewards, masks, old_logits, bad_masks=None, active_masks=None, available_actions=None):
        """
        ...
        self.old_logits[self.step] = old_logits.copy()
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        #print("minibatch size is", num_envs_per_batch)
        ...
        #print("obs shape before", obs.shape)
        ...
        old_logits = self.old_logits.reshape(-1, batch_size, self.old_logits.shape[-1])
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                old_logits_batch.append(old_logits[:, ind])
            ...
            #print("share obs batch inside buffer shape", share_obs_batch.shape)
            obs_batch = np.stack(obs_batch, 1)
            actions_batch = np.stack(actions_batch, 1)
            if self.available_actions is not None:
                available_actions_batch = np.stack(available_actions_batch, 1)
            old_logits_batch = np.stack(old_logits_batch, 1)
            ...
            old_logits_batch = _flatten(T, N, old_logits_batch)    
            value_preds_batch = _flatten(T, N, value_preds_batch)
            return_batch = _flatten(T, N, return_batch)
            masks_batch = _flatten(T, N, masks_batch)
            active_masks_batch = _flatten(T, N, active_masks_batch)
            old_action_log_probs_batch = _flatten(T, N, old_action_log_probs_batch)
            #logits_batch = flatten
            adv_targ = _flatten(T, N, adv_targ)

            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                  value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch,\
                  adv_targ, available_actions_batch, old_logits_batch 
