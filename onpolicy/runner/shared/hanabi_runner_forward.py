...
class HanabiRunner(Runner):
    ...
    def run(self):
        '''
        share_obs: joint observations.
        obs: local agent observations.
        rnn_states: RNN states for actor network.
        rnn_states_critic: RNN states for critic network.
        actions: actions taken by agents.
        action_log_probs: log probs of actions taken by agents
        rewards:  reward collected at each step.
        masks: denotes whether the environment has terminated or not.
        bad_masks: denotes indicate whether whether true terminal state or due to episode limit
        active_masks: denotes whether an agent is active or dead in the env.
        available_actions: actions available to each agent. If None, all actions are available.
        logits: Logits of actions
        '''
        ...
        self.turn_logits = np.zeros((self.n_rollout_threads,*self.buffer.old_logits.shape[2:]), dtype=np.float32)
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                # print("reset choose", self.reset_choose)  
    ...
    def collect(self, step):
        for current_agent_id in range(self.num_agents):
            ...
            choose = np.any(self.use_available_actions == 1, axis=1) #maybe seeing if there are any available actions
            if ~np.any(choose):
                self.reset_choose = np.ones(self.n_rollout_threads) == 1.0
                #when reset choose is activated for an agent, it's action is not executed and we break out of the action loop back to the caller function
                break
            
            self.trainer.prep_rollout()

            value, action, action_log_prob, rnn_state, rnn_state_critic, logits \
            ...
            self.turn_logits[choose, current_agent_id] =  _t2n(logits)
            ...
            # rearrange reward #todo
            # reward of step 0 will be thrown away.
            # for the agent that did not take an action in the last step, turn reward at the current step will be turn rewards accummulated since the last time it took an action
            #after setting this, the turn rewards for that agent are set to 0 and the turn rewards for all agents are update again.
